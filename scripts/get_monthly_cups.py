...
def get_best_cup_dict(tank_id, verbose = False):
    ...
    if i_cup_dict['total'] > 0:
        ...
        try:
            ...
            pass
    if verbose == True:
        print('(tank_id: ' + str(tank_id) + ')', '- Number of other tanks:', j_count, i_or_j_str)
    return(best_cup_dict)

def find_signature_tank_id(tank_id, signature_cup_df, verbose = False):
    best_cup_dict = get_best_cup_dict(tank_id, verbose)
    try:
