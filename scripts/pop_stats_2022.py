...
if __name__ == '__main__':
    ...
    # dfs
    deaths = y2022_tanks.sort_values('deactivated', ascending = False).reset_index(drop = True).head(pop_stats_overall_max_display)
    deaths = deaths[deaths['deactivated'] != 0].reset_index(drop = True)
    kills = y2022_tanks.sort_values('destroyed_enemies', ascending = False).reset_index(drop = True).head(pop_stats_overall_max_display)
    kills = kills[kills['destroyed_enemies'] != 0].reset_index(drop = True)
    playtime = y2022_tanks.sort_values('total_seconds', ascending = False).reset_index(drop = True).head(pop_stats_overall_max_display)
    playtime = playtime[playtime['total_seconds'] != 0].reset_index(drop = True)
    # pop
    with open(pop_stats_2022_md, 'w') as f:
